 ...
 * used in this bolt. KnowledgeAgent is used to monitor the knowledge base and
 * if rules have been changed, it will update automatically
 * 
 * 
 * @author Tony WANG
 * 
...
public class ProcessingBolt extends BaseBasicBolt {
		...
		try {
			...
			while (msgIt.hasNext()) {
				...
				if (!setFactTypeFromMap(message, jsonMap, factTypes))
					list.add("{\"FactTypeAlarm\":\"" + "Message with KEYS:"
							+ jsonMap.keySet().toString()
	...
	private static KnowledgeAgent createKnowledgeAgent() {
		...
		xml += "    <add> ";
		xml += "        <resource source='http://"
				+ DSPPropertyUtil.KNOWLEDGE_SERVER_IP
				+ ":8080/guvnor/org.drools.guvnor.Guvnor/package/"
				+ DSPPropertyUtil.KNOWLEDGE_PACKAGE_NAME
...
//		System.out.println(xml);
		...
		// Resource changeset =
		// ResourceFactory.newInputStreamResource(ClassLoader
		// .getSystemResourceAsStream("ChangeSet.xml"));
		Resource changeset = ResourceFactory.newInputStreamResource(is);
	...
	public static void main(String[] args) throws InterruptedException {
		String json1 = "{\"point_name\":\"dciPOW-HOSTAT\",\"description\":\"35kV\",\"type\":\"DI1\",\"value\":2,\"acquisitionTime\":\"/Date(1343899543323)/\",\"optional1\":\"Message\"}";
